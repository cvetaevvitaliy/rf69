...
volatile RFM69::RadioAddrType RFM69::SENDERID;
volatile RFM69::RadioAddrType RFM69::TARGETID;     // should match _address
...
bool RFM69::initialize(uint8_t freqBand, RadioAddrType nodeID, uint8_t networkID) {
	...
	/* 0x02 */{ REG_DATAMODUL, RF_DATAMODUL_DATAMODE_PACKET | RF_DATAMODUL_MODULATIONTYPE_FSK
			...
			/* 0x37 */{ REG_PACKETCONFIG1, RF_PACKET1_FORMAT_VARIABLE | RF_PACKET1_DCFREE_OFF | RF_PACKET1_CRC_ON
					| RF_PACKET1_CRCAUTOCLEAR_ON | RF_PACKET1_ADRSFILTERING_OFF },
			/* 0x38 */{ REG_PAYLOADLENGTH, 66 }, // in variable length mode: the max frame size, not used in TX
			///* 0x39 */ { REG_NODEADRS, nodeID }, // turned off because we're not using address filtering
			/* 0x3C */{ REG_FIFOTHRESH, RF_FIFOTHRESH_TXSTART_FIFONOTEMPTY | RF_FIFOTHRESH_VALUE }, // TX on FIFO not empty
			/* 0x3D */{ REG_PACKETCONFIG2, RF_PACKET2_RXRESTARTDELAY_2BITS | RF_PACKET2_AUTORXRESTART_ON
...
//CMDC0DE
	...
	if (millis() - start >= timeout)
		return false;
	_inISR = false;
//	attachInterrupt(_interruptNum, RFM69::isr0, 0);
...
uint32_t RFM69::getFrequency() {
	return RF69_FSTEP
			* (((uint32_t) readReg(REG_FRFMSB) << 16) + ((uint16_t) readReg(REG_FRFMID) << 8) + readReg(REG_FRFLSB));
}
...
bool RFM69::sendWithRetry(RadioAddrType toAddress, const void* buffer, uint8_t bufferSize, uint8_t retries,
...
bool RFM69::ACKReceived(RadioAddrType fromNodeID) {
	if (receiveDone())
...
void RFM69::sendFrame(RFM69::RadioAddrType toAddress, const void* buffer, uint8_t bufferSize, bool requestACK,
	...
	// write to FIFO
	select();
	transfer(REG_FIFO | 0x80);
	transfer(bufferSize + 5);
	transfer((toAddress & 0xFF00) >> 8);
	transfer((toAddress & 0xFF));
	transfer((_address & 0xFF00) >> 8);
	transfer((_address & 0xFF));
	transfer(CTLbyte);

	for (uint8_t i = 0; i < bufferSize; i++)
		transfer(((uint8_t*) buffer)[i]);
...
void RFM69::interruptHandler() {
	...
	if (_mode == RF69_MODE_RX && (readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PAYLOADREADY)) {
		...
		setMode(RF69_MODE_STANDBY);
		select();
		transfer(REG_FIFO & 0x7F);
		PAYLOADLEN = transfer(0);
		PAYLOADLEN = PAYLOADLEN > 66 ? 66 : PAYLOADLEN; // precaution
		TARGETID = transfer(0) << 8;
		TARGETID |= transfer(0);
		...
		DATALEN = PAYLOADLEN - 5;
		SENDERID = transfer(0) << 8;
		SENDERID |= transfer(0);
		uint8_t CTLbyte = transfer(0);
